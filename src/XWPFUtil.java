 ...
 * }
 * 
 * public static XWPFParagraph insertAfter(XWPFParagraph paragraph, XWPFDocument
 * doc) {
 ...
 * }
 * 
 * public static XWPFParagraph insertBefore(XWPFParagraph paragraph,
 * XWPFDocument doc) {
 ...
 * }
 * 
 * private static void collectAllCharacters(StringBuilder text, List<XWPFRun>
 * runs) {
 * if (runs != null) {
 * for (XWPFRun r : runs) {
 ...
 * }
 * 
 * public static void replaceMark(XWPFDocument doc, Map<String, String>
 * replacements) {
 ...
 * }
 * 
 * public static void replaceInTables(XWPFDocument doc, Map<String, String>
 * replacements) {
 * for (XWPFTable xwpfTable : doc.getTables()) {
 * List<XWPFTableRow> tableRows = xwpfTable.getRows();
 * for (XWPFTableRow xwpfTableRow : tableRows) {
 ...
 * }
 * 
 * public static void replaceInHeader(XWPFDocument doc, Map<String, String>
 * replacements) {
 * for (XWPFHeader header : doc.getHeaderList())
 * replaceInParagraphs(header.getParagraphs(), replacements);
 * }
 * 
 * public static void replaceInFooter(XWPFDocument doc, Map<String, String>
 * replacements) {
 * for (XWPFFooter footer : doc.getFooterList())
 * replaceInParagraphs(footer.getParagraphs(), replacements);
 * }
 * 
 * public static void replaceInParagraphs(List<XWPFParagraph> paragraphs,
 ...
 * public static void replaceInParagraphs(List<XWPFParagraph> paragraphs,
 * Map<String, String> replacements,
 * Character startCharacter, Character secondStartCharacter, Character
 * endCharacter) {
 * mergeTextForVariables(paragraphs, startCharacter, secondStartCharacter,
 * endCharacter);
 ...
 * String searchText = startCharacter.toString() +
 * secondStartCharacter.toString()
 ...
 * }
 * 
 * private static void mergeTextForVariables(List<XWPFParagraph> paragraphs,
 * Character startCharacter,
 * Character secondStartCharacter, Character endCharacter) {
 * for (List<XWPFRun> list : collectSeparatedRuns(paragraphs, startCharacter,
 * secondStartCharacter,
 * endCharacter)) {
 ...
 * firstRun.setText(firstRun.getText(-1) + lastRunText.substring(0,
 * lastRunText.indexOf(endCharacter) + 1), 0);
 * lastRun.setText(lastRunText.substring(lastRunText.indexOf(endCharacter) + 1),
 * 0);
 * }
 * }
 * 
 * private static List<List<XWPFRun>> collectSeparatedRuns(List<XWPFParagraph>
 * paragraphs, Character startCharacter,
 * Character secondStartCharacter, Character endCharacter) {
 ...
 * for (XWPFRun run : paragraph.getRuns()) {
 * String text = run.getText(-1);
 * DetectionType type = detectRun(run, currentType, lastCharacter,
 * startCharacter, secondStartCharacter,
 * endCharacter);
 * if (text != null) {
 * if (text.length() > 1)
 ...
 * tempList.add(run);
 * } else if (currentType == DetectionType.CONTINUE && type ==
 * DetectionType.CONTINUE) {
 * tempList.add(run);
 * } else if (currentType == DetectionType.CONTINUE && type ==
 * DetectionType.NONE) {
 * tempList.clear();
 ...
 * }
 * 
 * private static DetectionType detectRun(XWPFRun run, DetectionType type,
 ...
 * return DetectionType.NONE;
 * if (isOpen(text, false, null, startCharacter, secondStartCharacter,
 * endCharacter))
 * return DetectionType.CONTINUE;
 * return DetectionType.END;
 * } else if (type == DetectionType.CONTINUE) {
 * if (isOpen(text, true, lastCharacter, startCharacter, secondStartCharacter,
 * endCharacter))
 * return DetectionType.CONTINUE;
 ...
 * }
 * 
 * private static boolean isOpen(String text, boolean state, Character
 * lastCharacter, Character startCharacter,
 * Character secondStartCharacter, Character endCharacter) {
 * boolean open = state;
